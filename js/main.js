...
export function geturl() {
  let url = {};
  url.href = new URL(document.location.href); 
  url.paramsString = document.location.search; 
  url.searchParams = new URLSearchParams(url.paramsString); 
...
// вспомогательная функция: добавить атрибуты
function addAttr(elt,e) {
  if (!!e.editable) elt.attr('contenteditable', e.editable);
  if (!!e.pholder) elt.attr('placeholder', e.pholder);
  if (!!e.changec) elt.attr('change-control', e.changec);
  if (!!e.close) elt.attr('close', e.close);
  if (!!e.switch) elt.attr('switch', e.switch);
  if (!!e.control) elt.attr('control', e.control);
  if (!!e.href) elt.attr('href', e.href);
  if (!!e.target) elt.attr('target', e.target);
  if (!!e.datatoggle) elt.attr('data-toggle', e.datatoggle);
  if (!!e.dataplacement) elt.attr('data-placement', e.dataplacement);
  if (!!e.title) elt.attr('title', e.title);
  if (!!e.itype) elt.attr('type', e.itype);
  if (!!e.value) elt.attr('value', e.value);
  if (!!e.val) elt.val(e.val);
  if (!!e.checked) elt.attr('checked',e.checked);
  if (!!e.for) elt.attr('for', e.for);
  if (!!e.name) elt.attr('name', e.name);
  if (!!e.answer) elt.attr('answer', e.answer);
  if (!!e.disabled) elt.attr('disabled', e.disabled);
  if (!!e.formula) elt.attr('formula', e.formula);
  if (!!e.ops) elt.attr('ops', e.ops);
  if (!!e.datainputmask) elt.attr('data-inputmask', e.datainputmask);
  if (!!e.datatoggle) elt.attr('data-toggle', e.datatoggle);
  if (!!e.datasize) elt.attr('data-size', e.datasize);
  if (!!e.datawidth) elt.attr('data-width', e.datawidth);
  if (!!e.dataheight) elt.attr('data-height', e.dataheight);
  if (!!e.datastyle) elt.attr('data-style', e.datastyle);
  if (!!e.dataonstyle) elt.attr('data-onstyle', e.dataonstyle);
  if (!!e.dataoffstyle) elt.attr('data-offstyle', e.dataoffstyle);
  if (!!e.dataon) elt.attr('data-on', e.dataon);
  if (!!e.dataoff) elt.attr('data-off', e.dataoff);
/*  
  ...
  const frmJson = $.getJSON(frmfile)
    .done(function(data) {
      ...
      if (frm.ver == "0.0.0 b") {
        ...
        $.each(frm.settings, function(key, val) {
          log('settings: ' + key + ':' + val);
          // end проход по разделу с настройкамм
        });
...
*/
        ...
        // проход по разделу свойств
        $.each(frm.properties, function(key, val) {
          log('properties: ' + key + ':' + val);
          let id = val.id;
          let lbl = val.lbl;
          let on = val.on;
          let attr = val.attr;
          // добавить обработчик поля при изменении
          // добавить обработчик для списка
          // добавить обработчик для toggle
          // добавить обработчик для editable
          
          // end проход по разделу свойств
        });
